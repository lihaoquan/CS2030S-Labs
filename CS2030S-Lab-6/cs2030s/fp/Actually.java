...
public abstract class Actually<T> implements Immutatorable<T>, Actionable<T> {
  ...
  private static class Success<T> extends Actually<T> {
    ...
    public <U> Actually<U> next(Immutator<? extends Actually<? extends U>, ? super T> param) {
      try {
        @SuppressWarnings("unchecked")
        Actually<U> retval = (Actually<U>) param.invoke(this.value);
        return retval;
      } catch (Exception e) {
        return Actually.err(e);
      }
    ...
    /**
     * Check for equality between instances of {@code Success<T>}.
     *
  ...
  private static class Failure extends Actually<Object> {
    ...
    public boolean equals(Object obj) {
      ...
      if (obj instanceof Failure) {
        Failure tmp = (Failure) obj;
        if (this.exc == tmp.exc) {
          return true;
        }
        if (this.exc == null || tmp == null || tmp.exc == null || this.exc.getMessage() == null) {
          return false;
        }
        return this.exc.getMessage().equals(tmp.exc.getMessage());
      }
      return false;
